  ...
  let infoTable = [
    {
      head: "ردیف",
      field: "rowTable",
      clickTr: (info) => console.log("info", info),
    },
    {
      head: "شماره درخواست",
      field: "requestNo",
    ...
    },
    {
      ...
      selector: (data) => (
        <div
          className={`py-[1px] px-2 border rounded ${
            data?.state === 1
              ? "bg-[#DCE5FC] border-[#0468DB] text-[#0468DB]"
              : data?.state === 2
              ? "bg-green-100 border-green-500 text-green-500"
              : "bg-red-100 border-red-500 text-red-500"
          }`}
        >
          {data?.throughTransportState}
        </div>
      ),
    },
    {
      ...
      selector: (data) => (
        <div
          ...
          onClick={() => navigate(`/cartable/shipping/${data}`)}
        ></div>
      ),
    },
  ];
